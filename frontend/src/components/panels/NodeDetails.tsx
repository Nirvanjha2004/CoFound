...
function overviewText(node: GraphNode, latest?: GraphNode['research_history'][number]) {
  const raw = node.overview ?? node.summary
  const failed =
    latest?.status === 'failed' ||
    latest?.status === 'partial' ||
    /not accepted|research failed|unavailable|lacks specific/i.test(`${latest?.reason ?? ''} ${raw ?? ''}`)
  if (failed) {
    return 'Research failed on the last attempt. Approve research again to retry with fresh evidence.'
  }
  return raw
}
...
export function NodeDetails({ node }: NodeDetailsProps) {
  ...
  const overview = overviewText(node, latest)
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        {overview && (
          <div>
            ...
            <p className="text-sm leading-relaxed text-foreground/90">{overview}</p>
          </div>
        )}
