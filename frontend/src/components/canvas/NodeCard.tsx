...
function displaySummary(node: GraphNode): string {
  const latest = node.research_history.at(-1)
  const failed =
    latest?.status === 'failed' ||
    latest?.status === 'partial' ||
    /not accepted|research failed|unavailable|lacks specific/i.test(
      `${latest?.reason ?? ''} ${node.summary ?? ''}`,
    )

  if (failed) {
    return 'Research failed — open the node and approve again to retry.'
  }
  return node.summary || 'No summary yet.'
}
...
function NodeCardComponent({ data }: NodeProps & { data: NodeCardData }) {
  ...
  return (
    <div
      ...
      className={cn(
        'relative w-[240px] rounded-lg border bg-card p-3.5 transition-colors',
        isLocked
    ...
    >
      ...
      <p className="mb-2.5 line-clamp-2 text-[13px] text-foreground">{summary}</p>
