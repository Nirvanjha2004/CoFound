  ...
  const stopListening = useCallback(async () => {
    ...
    try {
      ...
      const voiceUnavailable =
        /deepgram|api key|not configured|503|502|401|403|failed/i.test(raw) || !raw
      appendOrchestratorMessage({
        role: 'agent',
        agentName: 'System',
        text: voiceUnavailable
          ? 'Voice is unavailable right now. Type your message instead.'
          : raw || 'Could not transcribe audio.',
      })
      setVoiceState('idle')
    }
